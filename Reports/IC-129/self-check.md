# IC-129 自验报告（v1，合并前版本）

> 报告提交方式说明（执行纪律第 7 条）：本报告须引用推送后才产生的 CI 运行编号与
> IPA 校验，故采用「同一张卡、同一分支内追加 docs 提交」的方式随卡交付；合并提交
> 短 SHA 与 G605 结果两项在合并完成后以 v2 完整替换版补记，不跨卡回填。

## 结论（先行）

**交付合格，全绿。** 对账依据已由「当前维度本次读到的范围集合」改为资产存在性
（叠加于既有按范围收敛之上，不替换）。CI #248 一次通过：XCTest **625 项 0 失败**
（较基数 619 增 6 项，即本卡六个新测试），真实退出码 0，目的地 iOS 26.2 /
iPhone 16，「XCTest 执行摘要」notice 在位。IC-127 两条回归断言原样保留并通过。
本地三条门禁退出码均为 0。CI 预算用 1 次（主跑），未动用修复预算。
...
合并前置核查与合并提交短 SHA、合并后 `main` 自动运行的编号与结果，见 v2 补记
（本 v1 提交时合并尚未执行）。
