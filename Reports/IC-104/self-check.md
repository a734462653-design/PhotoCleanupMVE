# IC-104 自验报告（A + B + C v4 全部交付，CI #189 绿）

覆盖 IC-104 全程：首轮停线（#178）、子项 A 收官（#180）、子项 B 收官（#181）、子项 C v1 规格冲突停线（#182）、子项 C v2 交付（#183 → #184）、子项 C v3 旧位锚定返工（#185 → #186 → #187 绿）、子项 C v4 带底缘视觉锚（#188 → **#189 绿**）。

## 结论（先行）

**A、B、C v4 三项全部交付，最终 CI 绿。Lynn 的复测包 = CI #189。**

- **最终绿 tip**①：`7295ed674bfee98cd6ef745854cae31c99ade7a8`
- **最终 CI**①：**#189 success**，9/9 步全绿，**真实退出码 0**，`Executed 520 tests, with 0 failures (0 unexpected) in 34.368 (45.739) seconds`
- **IPA**①：**837917 字节**，SHA-256 **`95009f48990e0dc5d22541a6e4e726474700a53f36697e7a12e603f3d472b643`**
- **判别实验结论**：#188 底角失败的 `first = 241` **实证为读法 A**（圆角弧外沿的抗锯齿像素落入探针死区）①——见「子项 C v4」第六节
- **闸门**：A1、B1、C1、C2 评估后均未触发；Cv2-1/2、Cv3-1/2、**Cv4-1/2** 通过；Cv2-3、Cv3-3、**Cv4-3** 的停线信号（`testIC100B2`）全程未触发；`g <= 0` 未触发
- **CI 预算**：本卡（v8+v9）2 次，**已用 2 次**（#188、#189）
- **时间闸门**：v9 续做开工 2026-08-28T15:55:31Z，到期 17:25:31Z，**完成于 16:12:08Z，未到期**
- **`main` 全程未被触碰**（停在 `e6bd5aa`）；`schemaVersion == 6`；冻结三链未动
- **IC-107（合并入 main）**：六点复测未通过，**本卡作废、未执行**

**Lynn 复测点**：**第 ② 点为主**（底距 = 顶距）；顺带 ⑤⑥（位移/跳变量已从 ~17.35 pt 缩到 ~10.35 pt）。
...
| run | id | 被测提交 | 结论 | XCTest | 退出码 | 说明 |
|---|---|---|---|---|---|---|
| #188 | `33184560783` | `4996873` | failure | 520 / 8 | 65 | 8 项全在 `testIC070G77...`，仅底部两角；顶部两角与四条边全通过 |
| **#189** | **`33187668094`** | **`7295ed6`** | **success** | **520 / 0** | **0** | 9/9 步全绿，IPA 已产出 |

### 六、判别实验：读法 A 实证成立①

#188 的 8 项失败全部集中在 `testIC070G77FitBorderIsConcentricAtCornersBeforeAndAfterToggle`（IC-070 描边同心性像素探针），仅**底部两角**，`first = 241`（要求 `< 234`）。

**两种互斥读法**：**A** 白 255 与描边（黑）的低覆盖抗锯齿混合（探针死区）；**B** 白 255 与照片 237 的混合（底角真实亚像素露边）。单凭 241 无法区分。

**判别方法（决策会话裁定）**：抬高探针背景阈值、**同时原样保留照片判定**（`first >= photoGray - 3` 即 `break`）。若真相为 B，跳过薄边后下一采样即照片 237，`first = 237` 仍不满足 `< 234`，测试必然照旧红——**B 不可能被掩盖**。

**结果**：阈值改后 CI **#189 全绿，520 项 0 失败**①。若为 B 则必红，故**读法 A 实证成立**：底角失败确系圆角弧外沿的抗锯齿像素落入探针死区，**产品描边渲染无误**。与 #188 中「顶部两角通过、四条边均量到 `edgeWidth = 1.0163` ~ 1pt 出厂线宽」的旁证一致。

### 七、阈值取值的更正（本会话自身错误，如实登记）

下发单 v9 依据我上一份报告写的「窗口 `238...250`、建议 `243`」定为 **243**。**该建议是错的，比较方向写反了**：

`borderScanReading` 的背景判定是 `gray >= 阈值`，**偏白才算背景**。故
- 要把实测 AA 像素 `241` 归为背景 => 必须 **阈值 <= 241**；
- 要不吞掉照片 `237` => 必须 **阈值 > 237**。

**有效窗口是 `238...241`。** 取 243 时 `241 >= 243` 为假，241 仍被记为 `firstNonBackground` 并立即 `break`，行为与原值 250 完全一致——CI 会以同样的 8 项、同样的 241 再红一次，既耗尽最后一次预算，又可能被误判为「读法 B 成立」（实测 241 与该分支所写的 `~237` 并不相符）。

推 CI 前已用数值验算逐个确认（250 / 243 无效；241 / 240 / 238 可用；237 危险）。**实装取窗口下端 238**，AA 相位容忍度最大，判别性不变。CI #189 转绿印证该取值正确。

### 八、C v4 最终改动

**产品侧（相对 #187 的 `0d461cc`）：仅 `S2Calibration.swift` 一个文件，22 增 4 删。**

- 新增 `S2ViewportLayout.stripVisualTopFromViewportBottom` = `stripBottomFromViewportBottom + 原始横栏高`（与渲染同源，不做 44 触控下限）
- `screenshotBandHeight` 的横栏顶缘由触控锚改为视觉锚
- `stripTopFromViewportBottom` / `resolvedStripHeight` 本体、渲染层、`S2View.swift`、`S2NativePhotoPager.swift`、chrome 布局**全部零 diff**；未新增可调参数

**测试侧**：两处触控锚 -> 视觉锚；新增一条锁定 `视觉锚 - 触控锚 == max(44, 横栏高) - 横栏高` 的断言；`borderScanReading` 背景阈值 `250 -> 238`（照片判定与全部 coverage 断言原样保留）。`testIC100B6`（toast，范围外）未动。**测试函数 520 不变。**
...
| **Cv4-1** | **通过** | #189 success、520 项 0 失败、真实退出码 0、IPA 已登记 |
| **Cv4-2** | **通过** | 产品 diff 仅 `S2Calibration.swift`；触控推导本体、渲染层、`S2View`、`Pager`、chrome 零 diff；`schemaVersion == 6`；冻结三链未动 |
| **Cv4-3** | **未触发停线** | #188 的 8 项全在 `testIC070G77...`（截图描边几何），`testIC100B2` 未失败 |
| 时间闸门（C v4，v9 1.5h） | **未到期** | 15:55:31Z 开工，16:12:08Z 完成，到期 17:25:31Z |
...
| 项 | 值 |
|---|---|
| **run 编号** | **#189**（`https://github.com/a734462653-design/PhotoCleanupMVE/actions/runs/33187668094`） |
| 被测提交 | `7295ed674bfee98cd6ef745854cae31c99ade7a8` |
| artifact | `PhotoCleanupMVE-unsigned-7295ed674bfe`（id `9692840064`，zip 838087 字节） |
| **IPA 字节数** | **837917** |
| **IPA SHA-256** | **`95009f48990e0dc5d22541a6e4e726474700a53f36697e7a12e603f3d472b643`** |
| 内含 | 子项 **A** + **B** + **C v4**，以及 IC-105 / IC-106 的测试侧修复；`schemaVersion == 6` |

**IPA 复核说明**：字节数与 SHA-256 取自 CI 内「未签名 IPA 校验」步骤注解（CI 侧实算，①）。本机未重下复核——Cv4-1 只要求登记，且 IPA 归档不可复现（IC-094/097 证据①），重下哈希不能作跨运行同一性判据。
...
0aa. **探针阈值取值的更正**：v9 卡内的 `243` 来自我上一份报告写反的比较方向，实际有效窗口为 `238...241`，已取 238 并由 CI #189 验证。见「子项 C v4」第七节。
0ab. **`testIC070G77` 的探针死区是既有缺陷，本卡只收窄未根除**：`borderScanReading` 原先在 234-249 之间留有 16 级判定死区，任何落入其中的抗锯齿像素都会让角扫描提前中止。阈值降到 238 后死区收窄为 234-237（4 级）。根因是测试照片灰度 237 与白 255 仅差 18 级，判别余量本就很窄；后续若再遇同类抖动，建议改用对比度更高的测试照片灰度。属探针方法学问题，不在本卡范围。
