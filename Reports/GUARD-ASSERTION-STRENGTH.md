...
| 分类 | 数量 | 判定口径 |
|---|---:|---|
| 强断言 | 23 | 测试把该单元格的事件提交给状态机 API，并断言状态机拒绝该事件。 |
| 弱断言 | 36 | 测试未把该单元格的事件提交给状态机并断言拒绝，只证明该事件与起始状态组合无法按现有入口构造。 |
| 已撤销 | 4 | 事件已随条款撤销（IC-134 F 整层撤销 L3），`S5Event` 中已无该事件，强弱分级失去对象。坐标保留至 SPEC-S5 v6 晋级。 |
| 合计 | 63 | 与追溯矩阵中的 63 个“断言型条款”一一对应。 |

> IC-138 B（2026-09-08）：上表原为强 26 / 弱 37。IC-134 F 撤销 L3 整层后，
> `用户点击“我已清空最近删除”` 事件的四个不可达单元格（C5-106、C5-108、C5-109、
> C5-110）不再可分级，其中三条原判“强”的依据（守卫测试提交该事件并断言拒绝）
> 已随事件删除而不成立，故改列“已撤销”。合计 63 不变——追溯矩阵的断言型条款
> 行一行未删，编号保持稳定。追溯矩阵中该事件另有一个可达单元格 C5-107，
> 不在本表 63 个不可达坐标之内，已在追溯矩阵侧同样标记为已撤销。

## 判定口径与证据边界

- 单元格坐标严格采用守卫测试 `TransitionCell.coordinate` 的定义：`规格文件|事件|起始状态`；表中同时保留追溯矩阵条款号。
- 守卫测试 `testAll115TransitionCellsAndEveryUnreachableCombination` 负责加载、遍历和分派单元格；强弱判定引用分派后的具体断言分支。
- IC-138 B：本节原按行号引用守卫测试（第 13–15 行、第 24–53 行）。行号随 IC-138 A 的改动移位，且行号引用本身会被后续任何编辑悄悄作废，故改为按符号名引用；逐单元格表中的行号引用保留原样，作为分级当时的证据快照，不随代码移动更新。
...
| C5-106<br><code>SPEC-S5-20260812.v5.md&#124;用户点击“我已清空最近删除”&#124;外部源</code> | 用户点击“我已清空最近删除” | 外部源 | 已撤销 | IC-134 F 整层撤销 L3 后 `S5Event` 已无该事件，类型层面提交不了，强弱分级失去对象。守卫测试按追溯矩阵判定理由里的「事件已撤销」标记跳过该单元格，坐标保留。 |
| C5-108<br><code>SPEC-S5-20260812.v5.md&#124;用户点击“我已清空最近删除”&#124;S5-C</code> | 用户点击“我已清空最近删除” | S5-C | 已撤销 | 同上。原分级为“强”，依据是守卫测试调用 `handle` 提交清空确认事件；该分支已随事件一并删除，旧依据不再成立。 |
| C5-109<br><code>SPEC-S5-20260812.v5.md&#124;用户点击“我已清空最近删除”&#124;S5-F</code> | 用户点击“我已清空最近删除” | S5-F | 已撤销 | 同上。原分级为“强”，依据是守卫测试调用 `handle` 提交清空确认事件；该分支已随事件一并删除，旧依据不再成立。 |
| C5-110<br><code>SPEC-S5-20260812.v5.md&#124;用户点击“我已清空最近删除”&#124;S5-U</code> | 用户点击“我已清空最近删除” | S5-U | 已撤销 | 同上。原分级为“强”，依据是守卫测试调用 `handle` 提交清空确认事件；该分支已随事件一并删除，旧依据不再成立。 |
