...
- 任务：`IC-20260812-011-spec-test-traceability`；测试基线：`5a76d734a9783e303ff44646928a687155946708`。
- 输入规格：`SPEC-S3-S4-20260812.v6.md` 与 `SPEC-S5-20260812.v5.md`。
- 纳入共同规则/共同不变量、各状态的显示元素/可用操作/禁用操作/迁出条件、状态迁移表全部数据单元格、提交快照与失败回调字段结构及字段不变量、跨页交接与出口契约。证据说明、验收回指、实测记录、未定项清单及其他复述段不重复编号。
- 状态迁移表按数据单元格编号；同一物理行的多个单元格分别占一个编号，原文字段均保留对应规格物理行全文，单元格坐标写入判定理由。所有“不可达”单元格均按断言型条款处理。
- 方法列只列直接相关的 XCTest 断言。若方法只覆盖复合条款的一部分，仍记录方法，但覆盖判定保守取“未覆盖”。
- `不适用` 仅使用题卡允许的四类理由；本矩阵没有以提高覆盖率为目的扩大 `不适用`。
...
C34-006	SPEC-S3-S4-20260812.v6.md	26	- “返回上游整理页”入口位于页面头部导航层，高于头部信息条与资产内容区；三个 S3 状态均显示该入口。触发后迁出 S3，目标为上游整理页；本文不定义上游整理页的内部目标状态。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-012	SPEC-S3-S4-20260812.v6.md	32	- S3 内的 `D` 只减不增；集合变大只能先返回上游整理页，再以新的 `D` 重新进入确认页。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-020	SPEC-S3-S4-20260812.v6.md	40	- 任何提交都必须来自 S3-2，并在发起删除请求之前冻结提交集合快照。	testCell14SubmitFromS3_2FreezesSnapshotForS4_1, testFreezeCountGuardRejectsEmptySetAndFormsNoSnapshot, testFreezeCompletionGuardRejectsS3_1AndFormsNoSnapshot	未覆盖	方法断言只有 S3-2 能形成快照，但未直接断言删除服务请求一定发生在快照冻结之后。
C34-021	SPEC-S3-S4-20260812.v6.md	46	- 页面头部导航层：页面标题、“确认删除”语义说明和“返回上游整理页”入口。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-029	SPEC-S3-S4-20260812.v6.md	57	- 使用页面头部导航层的入口返回上游整理页；该操作迁出 S3，目标为上游整理页。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-037	SPEC-S3-S4-20260812.v6.md	71	- 返回上游整理页：迁出 S3，目标为上游整理页；不定义其内部目标状态。		未覆盖	未发现直接断言该条款的 XCTest 方法。
C34-038	SPEC-S3-S4-20260812.v6.md	77	- 页面头部导航层：页面标题、最终确认说明和“返回上游整理页”入口。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-046	SPEC-S3-S4-20260812.v6.md	88	- 使用页面头部导航层的入口返回上游整理页；该操作迁出 S3，目标为上游整理页。		未覆盖	未发现直接断言该条款的 XCTest 方法。
C34-047	SPEC-S3-S4-20260812.v6.md	92	- 跳过快照冻结直接发起删除。	testCell14SubmitFromS3_2FreezesSnapshotForS4_1	未覆盖	方法断言正常路径会先冻结快照，未直接尝试绕过冻结调用删除服务。
...
C34-052	SPEC-S3-S4-20260812.v6.md	100	- 点击提交时快照冻结校验失败：不得发起删除；按共同状态判定规则迁至相应 S3 状态。	testFreezeCountGuardRejectsEmptySetAndFormsNoSnapshot, testFreezeCompletionGuardRejectsS3_1AndFormsNoSnapshot	未覆盖	方法验证两类守卫失败，但未复现从 S3-2 点击提交时的竞态校验失败及归一化迁移。
C34-053	SPEC-S3-S4-20260812.v6.md	101	- 返回上游整理页：迁出 S3，目标为上游整理页；不定义其内部目标状态。		未覆盖	未发现直接断言该条款的 XCTest 方法。
C34-054	SPEC-S3-S4-20260812.v6.md	107	- 页面头部导航层：页面标题和“返回上游整理页”入口。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-057	SPEC-S3-S4-20260812.v6.md	113	- 使用页面头部导航层的入口返回上游整理页；该操作迁出 S3，目标为上游整理页。		未覆盖	未发现直接断言该条款的 XCTest 方法。
...
C34-065	SPEC-S3-S4-20260812.v6.md	133	| 进入页面 | `n = 0` → S3-4；`n ≥ 1` 且有“未开始”或“进行中”项 → S3-1，只将“未开始”项入队；`n ≥ 1` 且无上述两种结论 → S3-2，直接复用缓存 | 不可达：S3-1 已表示页面完成进入 | 不可达：S3-2 已表示页面完成进入 | 不可达：S3-4 已表示页面完成进入 |		未覆盖	断言型条款（事件“进入页面” × 起始状态“S3-1 扫描中”）：未发现直接断言该单元格的 XCTest 方法。
C34-066	SPEC-S3-S4-20260812.v6.md	133	| 进入页面 | `n = 0` → S3-4；`n ≥ 1` 且有“未开始”或“进行中”项 → S3-1，只将“未开始”项入队；`n ≥ 1` 且无上述两种结论 → S3-2，直接复用缓存 | 不可达：S3-1 已表示页面完成进入 | 不可达：S3-2 已表示页面完成进入 | 不可达：S3-4 已表示页面完成进入 |		未覆盖	断言型条款（事件“进入页面” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。
C34-067	SPEC-S3-S4-20260812.v6.md	133	| 进入页面 | `n = 0` → S3-4；`n ≥ 1` 且有“未开始”或“进行中”项 → S3-1，只将“未开始”项入队；`n ≥ 1` 且无上述两种结论 → S3-2，直接复用缓存 | 不可达：S3-1 已表示页面完成进入 | 不可达：S3-2 已表示页面完成进入 | 不可达：S3-4 已表示页面完成进入 |		未覆盖	断言型条款（事件“进入页面” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。
C34-068	SPEC-S3-S4-20260812.v6.md	134	| 扫描完成 | 不可达：页面外没有 S3 扫描 | `D` 中已无“未开始”或“进行中”项 → S3-2 | 不可达：S3-2 的 `D` 已无未完成项 | 不可达：空集不启动扫描 |		未覆盖	断言型条款（事件“扫描完成” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-070	SPEC-S3-S4-20260812.v6.md	134	| 扫描完成 | 不可达：页面外没有 S3 扫描 | `D` 中已无“未开始”或“进行中”项 → S3-2 | 不可达：S3-2 的 `D` 已无未完成项 | 不可达：空集不启动扫描 |		未覆盖	断言型条款（事件“扫描完成” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。
C34-071	SPEC-S3-S4-20260812.v6.md	134	| 扫描完成 | 不可达：页面外没有 S3 扫描 | `D` 中已无“未开始”或“进行中”项 → S3-2 | 不可达：S3-2 的 `D` 已无未完成项 | 不可达：空集不启动扫描 |		未覆盖	断言型条款（事件“扫描完成” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。
C34-072	SPEC-S3-S4-20260812.v6.md	135	| 扫描中移除项 | 不可达：页面外没有移除入口 | 移除后 `n = 0` → S3-4；仍非空且 `D` 已无未完成项 → S3-2；否则 → S3-1；均保留该项缓存并按当前 `D` 重新求和 | 不可达：S3-2 没有进行中的扫描 | 不可达：空集没有进行中的扫描 |		未覆盖	断言型条款（事件“扫描中移除项” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-074	SPEC-S3-S4-20260812.v6.md	135	| 扫描中移除项 | 不可达：页面外没有移除入口 | 移除后 `n = 0` → S3-4；仍非空且 `D` 已无未完成项 → S3-2；否则 → S3-1；均保留该项缓存并按当前 `D` 重新求和 | 不可达：S3-2 没有进行中的扫描 | 不可达：空集没有进行中的扫描 |		未覆盖	断言型条款（事件“扫描中移除项” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。
C34-075	SPEC-S3-S4-20260812.v6.md	135	| 扫描中移除项 | 不可达：页面外没有移除入口 | 移除后 `n = 0` → S3-4；仍非空且 `D` 已无未完成项 → S3-2；否则 → S3-1；均保留该项缓存并按当前 `D` 重新求和 | 不可达：S3-2 没有进行中的扫描 | 不可达：空集没有进行中的扫描 |		未覆盖	断言型条款（事件“扫描中移除项” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。
C34-076	SPEC-S3-S4-20260812.v6.md	136	| 移除单项 | 不可达：页面外没有移除入口 | 移除后 `n = 0` → S3-4；仍非空且 `D` 已无未完成项 → S3-2；否则 → S3-1；均保留该项缓存并按当前 `D` 重新求和 | 移除后仍非空 → S3-2；移除最后一项 → S3-4；均保留该项缓存并按当前 `D` 重新求和 | 不可达：空集没有可移除项且操作禁用 |		未覆盖	断言型条款（事件“移除单项” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-079	SPEC-S3-S4-20260812.v6.md	136	| 移除单项 | 不可达：页面外没有移除入口 | 移除后 `n = 0` → S3-4；仍非空且 `D` 已无未完成项 → S3-2；否则 → S3-1；均保留该项缓存并按当前 `D` 重新求和 | 移除后仍非空 → S3-2；移除最后一项 → S3-4；均保留该项缓存并按当前 `D` 重新求和 | 不可达：空集没有可移除项且操作禁用 |	testDisabledOperationsInS3_4HaveNoEffect	已覆盖	断言型条款（事件“移除单项” × 起始状态“S3-4 空集”）：所列方法直接断言该单元格。
C34-080	SPEC-S3-S4-20260812.v6.md	137	| 全部取消 | 不可达：页面外没有全部取消入口 | → S3-4；所有资产的缓存结论保留 | → S3-4；所有资产的缓存结论保留 | 不可达：空集的全部取消操作禁用 |		未覆盖	断言型条款（事件“全部取消” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-083	SPEC-S3-S4-20260812.v6.md	137	| 全部取消 | 不可达：页面外没有全部取消入口 | → S3-4；所有资产的缓存结论保留 | → S3-4；所有资产的缓存结论保留 | 不可达：空集的全部取消操作禁用 |	testDisabledOperationsInS3_4HaveNoEffect	已覆盖	断言型条款（事件“全部取消” × 起始状态“S3-4 空集”）：所列方法直接断言该单元格。
C34-084	SPEC-S3-S4-20260812.v6.md	138	| 集合变为空 | 不可达：首次进入空集由“进入页面”事件路由 | → S3-4；所有资产的缓存结论保留 | → S3-4；所有资产的缓存结论保留 | 不可达：S3-4 的集合已经为空 |		未覆盖	断言型条款（事件“集合变为空” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-087	SPEC-S3-S4-20260812.v6.md	138	| 集合变为空 | 不可达：首次进入空集由“进入页面”事件路由 | → S3-4；所有资产的缓存结论保留 | → S3-4；所有资产的缓存结论保留 | 不可达：S3-4 的集合已经为空 |	testDisabledOperationsInS3_4HaveNoEffect	已覆盖	断言型条款（事件“集合变为空” × 起始状态“S3-4 空集”）：所列方法直接断言该单元格。
C34-088	SPEC-S3-S4-20260812.v6.md	139	| 点击提交 | 不可达：页面外没有提交入口 | 不可达：提交操作禁用，原因是扫描未完成 | 两项校验与原子冻结成功 → S4-1；校验失败则按共同状态判定规则回到相应 S3 状态 | 不可达：提交操作禁用，原因是集合为空 |		未覆盖	断言型条款（事件“点击提交” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。
C34-089	SPEC-S3-S4-20260812.v6.md	139	| 点击提交 | 不可达：页面外没有提交入口 | 不可达：提交操作禁用，原因是扫描未完成 | 两项校验与原子冻结成功 → S4-1；校验失败则按共同状态判定规则回到相应 S3 状态 | 不可达：提交操作禁用，原因是集合为空 |	testFreezeCompletionGuardRejectsS3_1AndFormsNoSnapshot	已覆盖	断言型条款（事件“点击提交” × 起始状态“S3-1 扫描中”）：所列方法直接断言该单元格。
C34-090	SPEC-S3-S4-20260812.v6.md	139	| 点击提交 | 不可达：页面外没有提交入口 | 不可达：提交操作禁用，原因是扫描未完成 | 两项校验与原子冻结成功 → S4-1；校验失败则按共同状态判定规则回到相应 S3 状态 | 不可达：提交操作禁用，原因是集合为空 |	testCell14SubmitFromS3_2FreezesSnapshotForS4_1	未覆盖	可达单元格（事件“点击提交” × 起始状态“S3-2 就绪”）：方法直接断言成功冻结路径，但未断言该单元格同时规定的 S3-2 竞态校验失败路径。
C34-091	SPEC-S3-S4-20260812.v6.md	139	| 点击提交 | 不可达：页面外没有提交入口 | 不可达：提交操作禁用，原因是扫描未完成 | 两项校验与原子冻结成功 → S4-1；校验失败则按共同状态判定规则回到相应 S3 状态 | 不可达：提交操作禁用，原因是集合为空 |	testFreezeCountGuardRejectsEmptySetAndFormsNoSnapshot	已覆盖	断言型条款（事件“点击提交” × 起始状态“S3-4 空集”）：所列方法直接断言该单元格。
...
C34-114	SPEC-S3-S4-20260812.v6.md	206	- 无会改变提交结果的应用内操作；系统级返回桌面或切换应用不属于应用内操作。	testDuplicateSubmissionIsRejected	未覆盖	方法仅断言一种会改变提交结果的重复提交操作被拒绝，未穷举该清单要求。
...
C34-116	SPEC-S3-S4-20260812.v6.md	211	- 修改、取消或拆分已提交快照。	testSnapshotNeverChangesInsideExecutionState	未覆盖	方法断言快照不变，但未分别断言取消与拆分入口不可用。
...
C34-128	SPEC-S3-S4-20260812.v6.md	239	- 无会改变提交结果的应用内操作。	testDuplicateSubmissionIsRejected, testSnapshotNeverChangesInsideExecutionState	未覆盖	方法未穷举 S4-2 中所有可能改变提交结果的应用内操作。
C34-129	SPEC-S3-S4-20260812.v6.md	243	- 再次提交同一快照。		未覆盖	未发现直接断言该条款的 XCTest 方法。
C34-130	SPEC-S3-S4-20260812.v6.md	244	- 修改、取消或拆分已提交快照。	testSnapshotNeverChangesInsideExecutionState	未覆盖	方法断言快照不变，但未分别断言取消与拆分入口不可用。
...
C34-146	SPEC-S3-S4-20260812.v6.md	277	- 若交接前应用处于非 active 或被终止：保持已持久化的 S4-E1；下次应用恢复 active 后继续迁至 S5-T0。	testReachable04InactiveFromSuccessTerminal, testReachable09ActiveFromSuccessTerminal, testReachable20TerminationFromSuccessTerminal	未覆盖	方法覆盖非 active 后交接与终止时保留终态，未直接断言重启后继续交接 S5-T0。
...
C34-152	SPEC-S3-S4-20260812.v6.md	294	- 修改三个结果集合。	testDisjointCompleteClassificationIsAccepted	未覆盖	方法断言回调分类原样保存，但未直接尝试修改已形成终态的三个集合。
C34-153	SPEC-S3-S4-20260812.v6.md	295	- 在 S4 内再次提交全部或部分资产。	testSecondStartWithSameSubmissionIdentifierIsRejected	未覆盖	方法断言同一提交标识不能再次整批启动，未断言部分资产再次提交。
...
C34-163	SPEC-S3-S4-20260812.v6.md	319	- 推断任一资产成功或失败。	testUnknownEntryDoesNotConstructClassificationSets	未覆盖	方法断言 S5 未知入口不构造分类集合，但未直接尝试在 S4-E3 推断单项成功或失败。
C34-164	SPEC-S3-S4-20260812.v6.md	320	- 修改提交集合快照。	testSnapshotNeverChangesInsideExecutionState	已覆盖	所列方法直接断言该条款。
C34-165	SPEC-S3-S4-20260812.v6.md	321	- 在 S4 内再次提交全部或部分资产。	testSecondStartWithSameSubmissionIdentifierIsRejected	未覆盖	方法断言完整快照不能再次提交，未直接断言部分资产再次提交也被拒绝。
C34-166	SPEC-S3-S4-20260812.v6.md	322	- 接受迟到回调改写已封闭终态。	testLateCallbackCannotOverwriteUnknown	未覆盖	方法断言迟到成功回调不能改写未知终态，未直接断言迟到失败回调。
...
C34-171	SPEC-S3-S4-20260812.v6.md	337	| 提交发起 | 快照冻结成功并发起批次请求 → S4-1，立即开始累计 60 秒 | 不可达：同一提交标识禁止重复发起 | 不可达：恢复期间禁止重复发起 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 |	testDuplicateSubmissionIsRejected, testSecondStartWithSameSubmissionIdentifierIsRejected	已覆盖	断言型条款（事件“提交发起” × 起始状态“S4-1 已提交”）：所列方法直接断言该单元格。
C34-172	SPEC-S3-S4-20260812.v6.md	337	| 提交发起 | 快照冻结成功并发起批次请求 → S4-1，立即开始累计 60 秒 | 不可达：同一提交标识禁止重复发起 | 不可达：恢复期间禁止重复发起 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 |		未覆盖	断言型条款（事件“提交发起” × 起始状态“S4-2 已恢复交互”）：未发现直接断言该单元格的 XCTest 方法。
C34-173	SPEC-S3-S4-20260812.v6.md	337	| 提交发起 | 快照冻结成功并发起批次请求 → S4-1，立即开始累计 60 秒 | 不可达：同一提交标识禁止重复发起 | 不可达：恢复期间禁止重复发起 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 |		未覆盖	断言型条款（事件“提交发起” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。
C34-174	SPEC-S3-S4-20260812.v6.md	337	| 提交发起 | 快照冻结成功并发起批次请求 → S4-1，立即开始累计 60 秒 | 不可达：同一提交标识禁止重复发起 | 不可达：恢复期间禁止重复发起 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 |		未覆盖	断言型条款（事件“提交发起” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。
C34-175	SPEC-S3-S4-20260812.v6.md	337	| 提交发起 | 快照冻结成功并发起批次请求 → S4-1，立即开始累计 60 秒 | 不可达：同一提交标识禁止重复发起 | 不可达：恢复期间禁止重复发起 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 | 不可达：终态已经封闭提交 |		未覆盖	断言型条款（事件“提交发起” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。
C34-176	SPEC-S3-S4-20260812.v6.md	338	| 应用进入非 active | 不可达：S4 尚未开始 | → S4-1；暂停并保存已累计的 active 时长 | → S4-2；暂停并保存已累计的 active 时长 | → S4-E1；终态已持久化，等待应用恢复 active 后交接 | → S4-E2；终态已持久化，等待应用恢复 active 后交接 | → S4-E3；终态已持久化，等待应用恢复 active 后交接 |		未覆盖	断言型条款（事件“应用进入非 active” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-182	SPEC-S3-S4-20260812.v6.md	339	| 应用恢复 active | 不可达：S4 尚未开始 | → S4-2；从剩余时长继续累计 | → S4-2；从剩余时长继续累计 | → S4-E1，随后交接 S5-T0 | → S4-E2，随后按 `失败原因.类别码` 分流：`用户取消` → S5-C；`权限不足` / `资产不可删除` / `未知` → S5-F | → S4-E3，随后交接 S5-U |		未覆盖	断言型条款（事件“应用恢复 active” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-188	SPEC-S3-S4-20260812.v6.md	340	| 收到成功回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E1，并取消计时 | → S4-E1，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |		未覆盖	断言型条款（事件“收到成功回调” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-191	SPEC-S3-S4-20260812.v6.md	340	| 收到成功回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E1，并取消计时 | → S4-E1，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |		未覆盖	断言型条款（事件“收到成功回调” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。
C34-192	SPEC-S3-S4-20260812.v6.md	340	| 收到成功回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E1，并取消计时 | → S4-E1，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |	testLateSuccessCannotOverwriteFailure	已覆盖	断言型条款（事件“收到成功回调” × 起始状态“S4-E2 整批失败”）：所列方法直接断言该单元格。
C34-193	SPEC-S3-S4-20260812.v6.md	340	| 收到成功回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E1，并取消计时 | → S4-E1，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |	testLateCallbackCannotOverwriteUnknown	已覆盖	断言型条款（事件“收到成功回调” × 起始状态“S4-E3 结果未知”）：所列方法直接断言该单元格。
C34-194	SPEC-S3-S4-20260812.v6.md	341	| 收到失败回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E2，并取消计时 | → S4-E2，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，重复回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |		未覆盖	断言型条款（事件“收到失败回调” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-197	SPEC-S3-S4-20260812.v6.md	341	| 收到失败回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E2，并取消计时 | → S4-E2，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，重复回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |	testLateFailureCannotOverwriteSuccess	已覆盖	断言型条款（事件“收到失败回调” × 起始状态“S4-E1 全批成功”）：所列方法直接断言该单元格。
C34-198	SPEC-S3-S4-20260812.v6.md	341	| 收到失败回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E2，并取消计时 | → S4-E2，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，重复回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |		未覆盖	断言型条款（事件“收到失败回调” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。
C34-199	SPEC-S3-S4-20260812.v6.md	341	| 收到失败回调 | 不可达：S4 尚未开始且没有本次提交 | → S4-E2，并取消计时 | → S4-E2，并取消计时 | 不可达：首个终态已经持久化，迟到回调不得改写 | 不可达：首个终态已经持久化，重复回调不得改写 | 不可达：首个终态已经持久化，迟到回调不得改写 |		未覆盖	断言型条款（事件“收到失败回调” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。
C34-200	SPEC-S3-S4-20260812.v6.md	342	| 超时触发 | 不可达：S4 尚未开始 | active 累计达到 60 秒且无有效回调 → S4-E3 | active 累计达到 60 秒且无有效回调 → S4-E3 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 |		未覆盖	断言型条款（事件“超时触发” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-203	SPEC-S3-S4-20260812.v6.md	342	| 超时触发 | 不可达：S4 尚未开始 | active 累计达到 60 秒且无有效回调 → S4-E3 | active 累计达到 60 秒且无有效回调 → S4-E3 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 |		未覆盖	断言型条款（事件“超时触发” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。
C34-204	SPEC-S3-S4-20260812.v6.md	342	| 超时触发 | 不可达：S4 尚未开始 | active 累计达到 60 秒且无有效回调 → S4-E3 | active 累计达到 60 秒且无有效回调 → S4-E3 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 |		未覆盖	断言型条款（事件“超时触发” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。
C34-205	SPEC-S3-S4-20260812.v6.md	342	| 超时触发 | 不可达：S4 尚未开始 | active 累计达到 60 秒且无有效回调 → S4-E3 | active 累计达到 60 秒且无有效回调 → S4-E3 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 | 不可达：终态已形成且计时器已取消 |		未覆盖	断言型条款（事件“超时触发” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。
C34-206	SPEC-S3-S4-20260812.v6.md	343	| 应用在此期间被系统终止 | 不可达：S4 尚未开始 | 若尚无持久化终态，下次启动 → S4-E3 | 若尚无持久化终态，下次启动 → S4-E3 | → S4-E1；持久化终态不丢失，下次启动继续交接 | → S4-E2；持久化终态不丢失，下次启动继续交接 | → S4-E3；持久化终态不丢失，下次启动继续交接 |		未覆盖	断言型条款（事件“应用在此期间被系统终止” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C34-209	SPEC-S3-S4-20260812.v6.md	343	| 应用在此期间被系统终止 | 不可达：S4 尚未开始 | 若尚无持久化终态，下次启动 → S4-E3 | 若尚无持久化终态，下次启动 → S4-E3 | → S4-E1；持久化终态不丢失，下次启动继续交接 | → S4-E2；持久化终态不丢失，下次启动继续交接 | → S4-E3；持久化终态不丢失，下次启动继续交接 |	testReachable20TerminationFromSuccessTerminal	未覆盖	可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E1 全批成功”）：方法断言终止时保留成功终态，但未直接断言下次启动继续交接。
C34-210	SPEC-S3-S4-20260812.v6.md	343	| 应用在此期间被系统终止 | 不可达：S4 尚未开始 | 若尚无持久化终态，下次启动 → S4-E3 | 若尚无持久化终态，下次启动 → S4-E3 | → S4-E1；持久化终态不丢失，下次启动继续交接 | → S4-E2；持久化终态不丢失，下次启动继续交接 | → S4-E3；持久化终态不丢失，下次启动继续交接 |	testReachable21TerminationFromFailureTerminal	未覆盖	可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E2 整批失败”）：方法断言终止时保留失败终态，但未直接断言下次启动继续交接。
C34-211	SPEC-S3-S4-20260812.v6.md	343	| 应用在此期间被系统终止 | 不可达：S4 尚未开始 | 若尚无持久化终态，下次启动 → S4-E3 | 若尚无持久化终态，下次启动 → S4-E3 | → S4-E1；持久化终态不丢失，下次启动继续交接 | → S4-E2；持久化终态不丢失，下次启动继续交接 | → S4-E3；持久化终态不丢失，下次启动继续交接 |	testReachable22TerminationFromUnknownTerminal	未覆盖	可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E3 结果未知”）：方法断言终止时保留未知终态，但未直接断言下次启动继续交接。
...
C5-016	SPEC-S5-20260812.v5.md	35	- 进入 `S5-T0` 时，应用内所有仍引用 `A` 的旧照片列表必须失效；后续重新进入照片列表时从照片库重新取得数据，不得用旧内存列表把这些资产重新显示在原位置。	testCell01SuccessEntry	未覆盖	方法断言持久化后使旧列表失效，但未断言后续照片列表必定重新读取照片库且不会复现旧内存资产。
...
C5-031	SPEC-S5-20260812.v5.md	165	- 定时或重复读取 `freeDiskStrictGB`。	testRepeatedConfirmationDoesNotReadAgain, testLifecycleEventsDoNotReadFreeDiskAgain, testUnavailableReadingsArePersistedWithoutDeltaOrRetry	未覆盖	方法断言重复操作、生命周期变化和不可用读数不会补读，但未直接模拟定时读取。
...
C5-034	SPEC-S5-20260812.v5.md	168	- 再次提交、修改提交集合，或通过“离开”以外的方式结束本次结果页。	testSuccessPageCannotReturnToConfirmation	未覆盖	方法只断言成功页不能返回确认页，未分别断言再次提交、修改提交集合及其他结束路径。
...
C5-069	SPEC-S5-20260812.v5.md	231	- 读取 `freeDiskStrictGB` 或显示 L3。	testL3DisplayRemainsBlockedForEveryState	未覆盖	方法断言失败页不显示 L3，但未直接断言失败页不读取 freeDiskStrictGB。
...
C5-086	SPEC-S5-20260812.v5.md	262	- 读取 `freeDiskStrictGB`，显示“我已清空最近删除”，或把人工核对结果写回为删除回调。	testConfirmationButtonIsEnabledOnlyBeforeSuccessConfirmation, testL3DisplayRemainsBlockedForEveryState	未覆盖	方法断言未知页无确认能力且不显示 L3，未直接断言不读取磁盘或不写回人工核对结果。
C5-087	SPEC-S5-20260812.v5.md	263	- 在 S5 内再次提交、修改 `P` 或返回确认页。	testUnknownPageCannotReturnToConfirmation	未覆盖	方法只断言未知页不能返回确认页，未分别断言再次提交和修改 P。
...
C5-092	SPEC-S5-20260812.v5.md	300	| 从 S4-E1 进入 | 成功终态与快照已持久化 → S5-T0；首次入场读取一次基线 | 不可达：S5-T0 已完成入场 | 不可达：取消页不接收成功终态 | 不可达：S4-E1 不是失败终态 | 不可达：S4-E1 不是未知终态 |		未覆盖	断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-T0”）：未发现直接断言该单元格的 XCTest 方法。
C5-093	SPEC-S5-20260812.v5.md	300	| 从 S4-E1 进入 | 成功终态与快照已持久化 → S5-T0；首次入场读取一次基线 | 不可达：S5-T0 已完成入场 | 不可达：取消页不接收成功终态 | 不可达：S4-E1 不是失败终态 | 不可达：S4-E1 不是未知终态 |		未覆盖	断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。
C5-094	SPEC-S5-20260812.v5.md	300	| 从 S4-E1 进入 | 成功终态与快照已持久化 → S5-T0；首次入场读取一次基线 | 不可达：S5-T0 已完成入场 | 不可达：取消页不接收成功终态 | 不可达：S4-E1 不是失败终态 | 不可达：S4-E1 不是未知终态 |		未覆盖	断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。
C5-095	SPEC-S5-20260812.v5.md	300	| 从 S4-E1 进入 | 成功终态与快照已持久化 → S5-T0；首次入场读取一次基线 | 不可达：S5-T0 已完成入场 | 不可达：取消页不接收成功终态 | 不可达：S4-E1 不是失败终态 | 不可达：S4-E1 不是未知终态 |		未覆盖	断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。
...
C5-097	SPEC-S5-20260812.v5.md	301	| 从 S4-E2 进入 | 按 S4 传入的 `下游目标状态` 直接落位，不得二次判定：`S5-C` → S5-C；`S5-F` → S5-F | 不可达：S4-E2 不进入成功路径 | 不可达：S5-C 已完成入场 | 不可达：S5-F 已完成入场 | 不可达：S4-E2 已有失败回调结论 |	testMismatchedHandoffPayloadAndTargetIsRejected	已覆盖	断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-T0”）：所列方法直接断言该单元格。
C5-098	SPEC-S5-20260812.v5.md	301	| 从 S4-E2 进入 | 按 S4 传入的 `下游目标状态` 直接落位，不得二次判定：`S5-C` → S5-C；`S5-F` → S5-F | 不可达：S4-E2 不进入成功路径 | 不可达：S5-C 已完成入场 | 不可达：S5-F 已完成入场 | 不可达：S4-E2 已有失败回调结论 |		未覆盖	断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。
C5-099	SPEC-S5-20260812.v5.md	301	| 从 S4-E2 进入 | 按 S4 传入的 `下游目标状态` 直接落位，不得二次判定：`S5-C` → S5-C；`S5-F` → S5-F | 不可达：S4-E2 不进入成功路径 | 不可达：S5-C 已完成入场 | 不可达：S5-F 已完成入场 | 不可达：S4-E2 已有失败回调结论 |		未覆盖	断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。
C5-100	SPEC-S5-20260812.v5.md	301	| 从 S4-E2 进入 | 按 S4 传入的 `下游目标状态` 直接落位，不得二次判定：`S5-C` → S5-C；`S5-F` → S5-F | 不可达：S4-E2 不进入成功路径 | 不可达：S5-C 已完成入场 | 不可达：S5-F 已完成入场 | 不可达：S4-E2 已有失败回调结论 |		未覆盖	断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。
C5-101	SPEC-S5-20260812.v5.md	302	| 从 S4-E3 进入 | 未知终态、触发原因与快照已持久化 → S5-U | 不可达：S4-E3 没有成功回调 | 不可达：S4-E3 没有取消回调 | 不可达：S4-E3 没有失败回调 | 不可达：S5-U 已完成入场 |	testCell03UnknownEntry	未覆盖	可达单元格（事件“从 S4-E3 进入” × 起始状态“外部源”）：方法断言未知原因和快照进入 S5-U，但未直接捕获并断言该入口已持久化。
C5-102	SPEC-S5-20260812.v5.md	302	| 从 S4-E3 进入 | 未知终态、触发原因与快照已持久化 → S5-U | 不可达：S4-E3 没有成功回调 | 不可达：S4-E3 没有取消回调 | 不可达：S4-E3 没有失败回调 | 不可达：S5-U 已完成入场 |		未覆盖	断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-T0”）：未发现直接断言该单元格的 XCTest 方法。
C5-103	SPEC-S5-20260812.v5.md	302	| 从 S4-E3 进入 | 未知终态、触发原因与快照已持久化 → S5-U | 不可达：S4-E3 没有成功回调 | 不可达：S4-E3 没有取消回调 | 不可达：S4-E3 没有失败回调 | 不可达：S5-U 已完成入场 |		未覆盖	断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。
C5-104	SPEC-S5-20260812.v5.md	302	| 从 S4-E3 进入 | 未知终态、触发原因与快照已持久化 → S5-U | 不可达：S4-E3 没有成功回调 | 不可达：S4-E3 没有取消回调 | 不可达：S4-E3 没有失败回调 | 不可达：S5-U 已完成入场 |		未覆盖	断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。
C5-105	SPEC-S5-20260812.v5.md	302	| 从 S4-E3 进入 | 未知终态、触发原因与快照已持久化 → S5-U | 不可达：S4-E3 没有成功回调 | 不可达：S4-E3 没有取消回调 | 不可达：S4-E3 没有失败回调 | 不可达：S5-U 已完成入场 |		未覆盖	断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。
C5-106	SPEC-S5-20260812.v5.md	303	| 用户点击“我已清空最近删除” | 不可达：外部源没有该操作 | 读取一次完成读数、持久化展示结论 → S5-T0 | 不可达：取消页没有该操作 | 不可达：失败页没有该操作 | 不可达：未知页没有该操作 |		未覆盖	断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C5-108	SPEC-S5-20260812.v5.md	303	| 用户点击“我已清空最近删除” | 不可达：外部源没有该操作 | 读取一次完成读数、持久化展示结论 → S5-T0 | 不可达：取消页没有该操作 | 不可达：失败页没有该操作 | 不可达：未知页没有该操作 |		未覆盖	断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。
C5-109	SPEC-S5-20260812.v5.md	303	| 用户点击“我已清空最近删除” | 不可达：外部源没有该操作 | 读取一次完成读数、持久化展示结论 → S5-T0 | 不可达：取消页没有该操作 | 不可达：失败页没有该操作 | 不可达：未知页没有该操作 |		未覆盖	断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。
C5-110	SPEC-S5-20260812.v5.md	303	| 用户点击“我已清空最近删除” | 不可达：外部源没有该操作 | 读取一次完成读数、持久化展示结论 → S5-T0 | 不可达：取消页没有该操作 | 不可达：失败页没有该操作 | 不可达：未知页没有该操作 |		未覆盖	断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。
C5-111	SPEC-S5-20260812.v5.md	304	| 用户点击“返回确认页” | 不可达：外部源没有该操作 | 不可达：S5-T0 没有该操作 | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 不可达：S5-U 没有该操作 |		未覆盖	断言型条款（事件“用户点击“返回确认页”” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
C5-112	SPEC-S5-20260812.v5.md	304	| 用户点击“返回确认页” | 不可达：外部源没有该操作 | 不可达：S5-T0 没有该操作 | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 不可达：S5-U 没有该操作 |	testSuccessPageCannotReturnToConfirmation	已覆盖	断言型条款（事件“用户点击“返回确认页”” × 起始状态“S5-T0”）：所列方法直接断言该单元格。
...
C5-115	SPEC-S5-20260812.v5.md	304	| 用户点击“返回确认页” | 不可达：外部源没有该操作 | 不可达：S5-T0 没有该操作 | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 缓存存在 → S3-2；缓存已随会话清空 → S3-1；两条路径均携带 P | 不可达：S5-U 没有该操作 |	testUnknownPageCannotReturnToConfirmation	已覆盖	断言型条款（事件“用户点击“返回确认页”” × 起始状态“S5-U”）：所列方法直接断言该单元格。
C5-116	SPEC-S5-20260812.v5.md	305	| 用户离开页面 | 不可达：外部源尚无 S5 页面 | 结束本次清理会话 → S5-EXIT | 不可达：取消页只提供返回确认页 | 不可达：失败页只提供返回确认页 | 结束会话 → S5-EXIT |		未覆盖	断言型条款（事件“用户离开页面” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
C5-117	SPEC-S5-20260812.v5.md	305	| 用户离开页面 | 不可达：外部源尚无 S5 页面 | 结束本次清理会话 → S5-EXIT | 不可达：取消页只提供返回确认页 | 不可达：失败页只提供返回确认页 | 结束会话 → S5-EXIT |	testCell04LeaveFromSuccess	已覆盖	可达单元格（事件“用户离开页面” × 起始状态“S5-T0”）：所列方法直接断言该单元格。
C5-118	SPEC-S5-20260812.v5.md	305	| 用户离开页面 | 不可达：外部源尚无 S5 页面 | 结束本次清理会话 → S5-EXIT | 不可达：取消页只提供返回确认页 | 不可达：失败页只提供返回确认页 | 结束会话 → S5-EXIT |	testCancellationCannotLeaveThroughCompletionAction	已覆盖	断言型条款（事件“用户离开页面” × 起始状态“S5-C”）：所列方法直接断言该单元格。
C5-119	SPEC-S5-20260812.v5.md	305	| 用户离开页面 | 不可达：外部源尚无 S5 页面 | 结束本次清理会话 → S5-EXIT | 不可达：取消页只提供返回确认页 | 不可达：失败页只提供返回确认页 | 结束会话 → S5-EXIT |	testFailurePageCannotLeaveThroughCompletionAction	已覆盖	断言型条款（事件“用户离开页面” × 起始状态“S5-F”）：所列方法直接断言该单元格。
C5-120	SPEC-S5-20260812.v5.md	305	| 用户离开页面 | 不可达：外部源尚无 S5 页面 | 结束本次清理会话 → S5-EXIT | 不可达：取消页只提供返回确认页 | 不可达：失败页只提供返回确认页 | 结束会话 → S5-EXIT |	testCell06LeaveFromUnknown	已覆盖	可达单元格（事件“用户离开页面” × 起始状态“S5-U”）：所列方法直接断言该单元格。
C5-121	SPEC-S5-20260812.v5.md	306	| 应用进入非 active | 不可达：尚未进入 S5 | → S5-T0；不新增磁盘读取 | → S5-C；保留取消上下文 | → S5-F；保留失败上下文 | → S5-U；保留未知上下文 |		未覆盖	断言型条款（事件“应用进入非 active” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C5-126	SPEC-S5-20260812.v5.md	307	| 应用恢复 active | 不可达：尚未进入 S5；S4 的恢复交接由上游处理 | → S5-T0；不新增磁盘读取 | → S5-C；保留取消上下文 | → S5-F；保留失败上下文 | → S5-U；保留未知上下文 |		未覆盖	断言型条款（事件“应用恢复 active” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C5-131	SPEC-S5-20260812.v5.md	308	| 应用被系统终止 | 不可达：S5 尚未建立；若终止发生在 S4 则由 S4 规则处理 | 持久化 S5-T0 上下文；下次启动恢复 S5-T0 | 持久化 S5-C；下次启动恢复 S5-C | 持久化 S5-F；下次启动恢复 S5-F | 持久化 S5-U；下次启动恢复 S5-U |		未覆盖	断言型条款（事件“应用被系统终止” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。
...
C5-143	SPEC-S5-20260812.v5.md	327	- S5 本身不发起再次删除；后续提交仍须经过确认页冻结新的提交快照。	testSuccessPageCannotReturnToConfirmation, testUnknownPageCannotReturnToConfirmation, testCell14SubmitFromS3_2FreezesSnapshotForS4_1	未覆盖	方法覆盖返回路径限制与后续确认页重新冻结，但未直接尝试在 S5 内再次删除。
C5-144	SPEC-S5-20260812.v5.md	333	| S5-T0 | `S5-EXIT`，即应用的清理入口页 | 结束本次清理会话，清除本批 L3 读数与展示结论 |	testCell04LeaveFromSuccess, testPersistedSessionCarriesTargetReadingsDeltaAndDeclarationTime	未覆盖	方法断言成功页退出效果及 L3 字段可持久化，未断言退出时清除本批 L3 数据。
C5-145	SPEC-S5-20260812.v5.md	334	| S5-U | `S5-EXIT`，即应用的清理入口页 | 结束本次清理会话，不推断删除结果，不自动再次提交 |	testCell06LeaveFromUnknown, testUnknownEntryDoesNotConstructClassificationSets	未覆盖	方法断言未知页退出且不构造结果集合，未直接断言退出不会自动再次提交。
...
<!-- 反向映射开始 -->
```
XCTest 方法名	测试文件	行号	命中条款编号（无则留空）
...
| 条款编号 | 规格位置 | 判定理由 |
|---|---|---|
| C34-006 | `SPEC-S3-S4-20260812.v6.md:26` | 未发现直接断言该条款的 XCTest 方法。 |
...
| C34-012 | `SPEC-S3-S4-20260812.v6.md:32` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-020 | `SPEC-S3-S4-20260812.v6.md:40` | 方法断言只有 S3-2 能形成快照，但未直接断言删除服务请求一定发生在快照冻结之后。 |
| C34-021 | `SPEC-S3-S4-20260812.v6.md:46` | 未发现直接断言该条款的 XCTest 方法。 |
...
| C34-029 | `SPEC-S3-S4-20260812.v6.md:57` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-032 | `SPEC-S3-S4-20260812.v6.md:63` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-037 | `SPEC-S3-S4-20260812.v6.md:71` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-038 | `SPEC-S3-S4-20260812.v6.md:77` | 未发现直接断言该条款的 XCTest 方法。 |
...
| C34-046 | `SPEC-S3-S4-20260812.v6.md:88` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-047 | `SPEC-S3-S4-20260812.v6.md:92` | 方法断言正常路径会先冻结快照，未直接尝试绕过冻结调用删除服务。 |
| C34-048 | `SPEC-S3-S4-20260812.v6.md:93` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-052 | `SPEC-S3-S4-20260812.v6.md:100` | 方法验证两类守卫失败，但未复现从 S3-2 点击提交时的竞态校验失败及归一化迁移。 |
| C34-053 | `SPEC-S3-S4-20260812.v6.md:101` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-054 | `SPEC-S3-S4-20260812.v6.md:107` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-055 | `SPEC-S3-S4-20260812.v6.md:108` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-057 | `SPEC-S3-S4-20260812.v6.md:113` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-062 | `SPEC-S3-S4-20260812.v6.md:124` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-063 | `SPEC-S3-S4-20260812.v6.md:125` | 方法覆盖重新构造状态机后的分流与缓存复用，但未断言先迁出 S3 再携带新 D 返回的完整页面交接。 |
| C34-065 | `SPEC-S3-S4-20260812.v6.md:133` | 断言型条款（事件“进入页面” × 起始状态“S3-1 扫描中”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-066 | `SPEC-S3-S4-20260812.v6.md:133` | 断言型条款（事件“进入页面” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-067 | `SPEC-S3-S4-20260812.v6.md:133` | 断言型条款（事件“进入页面” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-068 | `SPEC-S3-S4-20260812.v6.md:134` | 断言型条款（事件“扫描完成” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-070 | `SPEC-S3-S4-20260812.v6.md:134` | 断言型条款（事件“扫描完成” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-071 | `SPEC-S3-S4-20260812.v6.md:134` | 断言型条款（事件“扫描完成” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-072 | `SPEC-S3-S4-20260812.v6.md:135` | 断言型条款（事件“扫描中移除项” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-074 | `SPEC-S3-S4-20260812.v6.md:135` | 断言型条款（事件“扫描中移除项” × 起始状态“S3-2 就绪”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-075 | `SPEC-S3-S4-20260812.v6.md:135` | 断言型条款（事件“扫描中移除项” × 起始状态“S3-4 空集”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-076 | `SPEC-S3-S4-20260812.v6.md:136` | 断言型条款（事件“移除单项” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-080 | `SPEC-S3-S4-20260812.v6.md:137` | 断言型条款（事件“全部取消” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-084 | `SPEC-S3-S4-20260812.v6.md:138` | 断言型条款（事件“集合变为空” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-088 | `SPEC-S3-S4-20260812.v6.md:139` | 断言型条款（事件“点击提交” × 起始状态“页面外”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-090 | `SPEC-S3-S4-20260812.v6.md:139` | 可达单元格（事件“点击提交” × 起始状态“S3-2 就绪”）：方法直接断言成功冻结路径，但未断言该单元格同时规定的 S3-2 竞态校验失败路径。 |
...
| C34-104 | `SPEC-S3-S4-20260812.v6.md:161` | 方法断言全字段冻结及失败时不形成快照，但未直接断言失败时删除服务没有收到请求。 |
...
| C34-114 | `SPEC-S3-S4-20260812.v6.md:206` | 方法仅断言一种会改变提交结果的重复提交操作被拒绝，未穷举该清单要求。 |
| C34-116 | `SPEC-S3-S4-20260812.v6.md:211` | 方法断言快照不变，但未分别断言取消与拆分入口不可用。 |
| C34-117 | `SPEC-S3-S4-20260812.v6.md:212` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-126 | `SPEC-S3-S4-20260812.v6.md:234` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-128 | `SPEC-S3-S4-20260812.v6.md:239` | 方法未穷举 S4-2 中所有可能改变提交结果的应用内操作。 |
| C34-129 | `SPEC-S3-S4-20260812.v6.md:243` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-130 | `SPEC-S3-S4-20260812.v6.md:244` | 方法断言快照不变，但未分别断言取消与拆分入口不可用。 |
...
| C34-146 | `SPEC-S3-S4-20260812.v6.md:277` | 方法覆盖非 active 后交接与终止时保留终态，未直接断言重启后继续交接 S5-T0。 |
...
| C34-152 | `SPEC-S3-S4-20260812.v6.md:294` | 方法断言回调分类原样保存，但未直接尝试修改已形成终态的三个集合。 |
| C34-153 | `SPEC-S3-S4-20260812.v6.md:295` | 方法断言同一提交标识不能再次整批启动，未断言部分资产再次提交。 |
| C34-154 | `SPEC-S3-S4-20260812.v6.md:296` | 未发现直接断言该条款的 XCTest 方法。 |
| C34-157 | `SPEC-S3-S4-20260812.v6.md:302` | 方法覆盖失败终态恢复与两类目标写入，但未直接断言终止重启后按已持久化目标继续且不重新分流。 |
...
| C34-163 | `SPEC-S3-S4-20260812.v6.md:319` | 方法断言 S5 未知入口不构造分类集合，但未直接尝试在 S4-E3 推断单项成功或失败。 |
| C34-165 | `SPEC-S3-S4-20260812.v6.md:321` | 方法断言完整快照不能再次提交，未直接断言部分资产再次提交也被拒绝。 |
| C34-166 | `SPEC-S3-S4-20260812.v6.md:322` | 方法断言迟到成功回调不能改写未知终态，未直接断言迟到失败回调。 |
| C34-168 | `SPEC-S3-S4-20260812.v6.md:327` | 方法分别断言未知终态被终止时保留及未闭合提交终止后可交接，但未直接覆盖已持久化 S4-E3 在重启后继续交接。 |
| C34-172 | `SPEC-S3-S4-20260812.v6.md:337` | 断言型条款（事件“提交发起” × 起始状态“S4-2 已恢复交互”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-173 | `SPEC-S3-S4-20260812.v6.md:337` | 断言型条款（事件“提交发起” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-174 | `SPEC-S3-S4-20260812.v6.md:337` | 断言型条款（事件“提交发起” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-175 | `SPEC-S3-S4-20260812.v6.md:337` | 断言型条款（事件“提交发起” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-176 | `SPEC-S3-S4-20260812.v6.md:338` | 断言型条款（事件“应用进入非 active” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-182 | `SPEC-S3-S4-20260812.v6.md:339` | 断言型条款（事件“应用恢复 active” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-186 | `SPEC-S3-S4-20260812.v6.md:339` | 可达单元格（事件“应用恢复 active” × 起始状态“S4-E2 整批失败”）：方法分别断言失败终态恢复和类别分流，但未直接覆盖取消终态在恢复 active 后按已持久化目标交接。 |
| C34-188 | `SPEC-S3-S4-20260812.v6.md:340` | 断言型条款（事件“收到成功回调” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-191 | `SPEC-S3-S4-20260812.v6.md:340` | 断言型条款（事件“收到成功回调” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-194 | `SPEC-S3-S4-20260812.v6.md:341` | 断言型条款（事件“收到失败回调” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-198 | `SPEC-S3-S4-20260812.v6.md:341` | 断言型条款（事件“收到失败回调” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-199 | `SPEC-S3-S4-20260812.v6.md:341` | 断言型条款（事件“收到失败回调” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-200 | `SPEC-S3-S4-20260812.v6.md:342` | 断言型条款（事件“超时触发” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-203 | `SPEC-S3-S4-20260812.v6.md:342` | 断言型条款（事件“超时触发” × 起始状态“S4-E1 全批成功”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-204 | `SPEC-S3-S4-20260812.v6.md:342` | 断言型条款（事件“超时触发” × 起始状态“S4-E2 整批失败”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-205 | `SPEC-S3-S4-20260812.v6.md:342` | 断言型条款（事件“超时触发” × 起始状态“S4-E3 结果未知”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-206 | `SPEC-S3-S4-20260812.v6.md:343` | 断言型条款（事件“应用在此期间被系统终止” × 起始状态“S3-2 外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C34-209 | `SPEC-S3-S4-20260812.v6.md:343` | 可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E1 全批成功”）：方法断言终止时保留成功终态，但未直接断言下次启动继续交接。 |
| C34-210 | `SPEC-S3-S4-20260812.v6.md:343` | 可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E2 整批失败”）：方法断言终止时保留失败终态，但未直接断言下次启动继续交接。 |
| C34-211 | `SPEC-S3-S4-20260812.v6.md:343` | 可达单元格（事件“应用在此期间被系统终止” × 起始状态“S4-E3 结果未知”）：方法断言终止时保留未知终态，但未直接断言下次启动继续交接。 |
| C34-230 | `SPEC-S3-S4-20260812.v6.md:381` | 未发现直接断言该条款的 XCTest 方法。 |
| C5-006 | `SPEC-S5-20260812.v5.md:23` | 方法覆盖四状态的可用出口，但未直接断言出口不会自动发起新提交或新扫描。 |
| C5-016 | `SPEC-S5-20260812.v5.md:35` | 方法断言持久化后使旧列表失效，但未断言后续照片列表必定重新读取照片库且不会复现旧内存资产。 |
...
| C5-031 | `SPEC-S5-20260812.v5.md:165` | 方法断言重复操作、生命周期变化和不可用读数不会补读，但未直接模拟定时读取。 |
| C5-033 | `SPEC-S5-20260812.v5.md:167` | 未发现直接断言该条款的 XCTest 方法。 |
| C5-034 | `SPEC-S5-20260812.v5.md:168` | 方法只断言成功页不能返回确认页，未分别断言再次提交、修改提交集合及其他结束路径。 |
| C5-035 | `SPEC-S5-20260812.v5.md:172` | 方法断言声明时间、完成读数和差值持久化并留在原状态，但未直接断言形成页面展示结论。 |
| C5-039 | `SPEC-S5-20260812.v5.md:176` | 方法分别覆盖终止保态、读数持久化、生命周期不补读与恢复，但未在同一路径断言已完成两次读取后的终止和重启。 |
...
| C5-069 | `SPEC-S5-20260812.v5.md:231` | 方法断言失败页不显示 L3，但未直接断言失败页不读取 freeDiskStrictGB。 |
...
| C5-086 | `SPEC-S5-20260812.v5.md:262` | 方法断言未知页无确认能力且不显示 L3，未直接断言不读取磁盘或不写回人工核对结果。 |
| C5-087 | `SPEC-S5-20260812.v5.md:263` | 方法只断言未知页不能返回确认页，未分别断言再次提交和修改 P。 |
| C5-092 | `SPEC-S5-20260812.v5.md:300` | 断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-T0”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-093 | `SPEC-S5-20260812.v5.md:300` | 断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-094 | `SPEC-S5-20260812.v5.md:300` | 断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-095 | `SPEC-S5-20260812.v5.md:300` | 断言型条款（事件“从 S4-E1 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-098 | `SPEC-S5-20260812.v5.md:301` | 断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-099 | `SPEC-S5-20260812.v5.md:301` | 断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-100 | `SPEC-S5-20260812.v5.md:301` | 断言型条款（事件“从 S4-E2 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-101 | `SPEC-S5-20260812.v5.md:302` | 可达单元格（事件“从 S4-E3 进入” × 起始状态“外部源”）：方法断言未知原因和快照进入 S5-U，但未直接捕获并断言该入口已持久化。 |
| C5-102 | `SPEC-S5-20260812.v5.md:302` | 断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-T0”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-103 | `SPEC-S5-20260812.v5.md:302` | 断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-104 | `SPEC-S5-20260812.v5.md:302` | 断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-105 | `SPEC-S5-20260812.v5.md:302` | 断言型条款（事件“从 S4-E3 进入” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-106 | `SPEC-S5-20260812.v5.md:303` | 断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-108 | `SPEC-S5-20260812.v5.md:303` | 断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-C”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-109 | `SPEC-S5-20260812.v5.md:303` | 断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-F”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-110 | `SPEC-S5-20260812.v5.md:303` | 断言型条款（事件“用户点击“我已清空最近删除”” × 起始状态“S5-U”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-111 | `SPEC-S5-20260812.v5.md:304` | 断言型条款（事件“用户点击“返回确认页”” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-116 | `SPEC-S5-20260812.v5.md:305` | 断言型条款（事件“用户离开页面” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-121 | `SPEC-S5-20260812.v5.md:306` | 断言型条款（事件“应用进入非 active” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-126 | `SPEC-S5-20260812.v5.md:307` | 断言型条款（事件“应用恢复 active” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-131 | `SPEC-S5-20260812.v5.md:308` | 断言型条款（事件“应用被系统终止” × 起始状态“外部源”）：未发现直接断言该单元格的 XCTest 方法。 |
| C5-143 | `SPEC-S5-20260812.v5.md:327` | 方法覆盖返回路径限制与后续确认页重新冻结，但未直接尝试在 S5 内再次删除。 |
| C5-144 | `SPEC-S5-20260812.v5.md:333` | 方法断言成功页退出效果及 L3 字段可持久化，未断言退出时清除本批 L3 数据。 |
| C5-145 | `SPEC-S5-20260812.v5.md:334` | 方法断言未知页退出且不构造结果集合，未直接断言退出不会自动再次提交。 |
<!-- 未覆盖清单结束 -->
