...
**G814 前置全部满足，已按卡内授权 `--no-ff` 合并入 `main` 并推送。**

| 项 | 值 |
|---|---|
| 合并前工作树 | `git status --porcelain` 空 |
| 合并前 `main` tip（本地与远端一致） | `8eebac1280cb9d697a88a5c7b62368c36bb864e1`（`git ls-remote origin refs/heads/main` 同值，未被他人推进） |
| 被合并分支 tip | `e272b05`（`4ca8770` 代码 + 报告提交） |
| **合并提交 SHA（`--no-ff`）** | **`6503e35c4fa3243871a185b5b4f6cc6c8dd707aa`** |
| 合并后 `main` | `6503e35`，已推送 |
| 冻结三链 + 探针分支（合并后复核） | `b368a6c` / `6736f1e` / `a7cc1ec` / `9db02b9`，均未变 |

### G815：合并后 `main` 的自动运行

| 项 | 值 |
|---|---|
| 运行编号 | **#278**（run id `34253588385`） |
| 被测提交 | `6503e35c4fa3243871a185b5b4f6cc6c8dd707aa`（合并提交） |
| 结论 | **success** |
| XCTest 项数 / 失败数 | **707 / 0**（notice：`Executed 707 tests, with 0 failures (0 unexpected) in 40.904 (51.187) seconds`） |
| 执行摘要 notice | 在位 |
...
| IPA | `PhotoCleanupMVE-unsigned.ipa`，字节数 **1324858**，SHA-256 `6189d1cb74bdce5fb6dfa4d79624f41148ec3988563f180d1b160e4606ee71bc` |

> IPA 字节数与 #277 完全一致（1324858），SHA-256 不同——归档不可复现是本仓既有已知性质（IC-094／097 实证），不作为跨运行同一性依据。

### 报告提交方式（纪律 7）

本卡采用**两段式**：`e272b05`（分支内 docs 提交，随代码一起推送，含 CI #277 的运行编号与 IPA 校验）+ 本节的回填提交（合并后落在 `main` 上，含合并提交 SHA 与 G815 的 #278）。两次都在**本卡范围内**完成，未跨卡回填。纯报告提交不触发 CI，是 `paths-ignore` 的预期行为。
