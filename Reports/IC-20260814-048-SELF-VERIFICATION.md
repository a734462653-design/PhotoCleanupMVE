...
运行态最终结论待本次提交触发的 macOS CI 回填。当前 Windows 主机没有 Xcode，本报告不会把静态检查写成 XCTest、Release 或 IPA 已通过。
...
当前本地结果：提交前模式执行 100 项检查，0 项失败，退出码 0；通用 `selfcheck.ps1` 与用户可见硬编码扫描均通过。

## 十、启动落点探针

探针条件是“若接入 S1 作为启动落点导致任何既有测试失败，立即停止”。本卡保持既有 `.upstream` 与 `.finished` 路由断言兼容，启动后的 S1 入口由新增 IC048-001 覆盖。最终是否触发探针必须以本次 macOS CI 的全量 273 项 XCTest 为准；CI 前不提前宣称通过。
