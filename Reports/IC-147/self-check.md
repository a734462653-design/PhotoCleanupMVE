...
13. **点「我已清空」后会**停在**「正在核对…」不再变化。** `beginVerification()` 把 `VF` 置为 `核对中`（这是本层该做的全部），而核对本体（阈值、轮询、`FREE` 取数、通过／未通过的判定）属 SPEC-S0 v1 第八节第 2 部分，排批次 5.3，本卡不实装——因此没有任何路径把 `VF` 推进到 `已通过`／`未通过`，也没有路径把它复位回 `无`（只有「打开应用」会，见第 8 条）。等待清空行与「我已清空」按钮同时仍在。**真机上这是预期的骨架行为，不是卡死**，H70 第 3 条判定「点得到」即可，不要据此判为缺陷。

14. **待删篮胶囊在真机上会读出「N · 零字节」。** 张数取会话层真值（在「逐张整理」里标记过照片后才出现），体积位取数据源而本卡注入的是桩（回报 0）。详见第 3 条。同样属预期的骨架读数。
