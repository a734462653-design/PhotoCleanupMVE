...
///
/// IC-162 时是视觉预览（新旧并存、由编译期开关切换）；IC-165 起「卡片叠」两页为正式页、旧页退役，
...
final class IC162DeckPreviewTests: XCTestCase {
    ...
    func testIC162A_CardsKeepOrderDropEmptyAndAppendRest() {
        ...
        // 入参四类，第二条 `candidateCount = 0` 被丢掉；「其余照片」行按入参序垫底（排序由状态机做）。
        XCTAssertEqual(
            cards.map { $0.id },
            [
                S0CategoryIdentifier.bigVideo.rawValue,
                S0CategoryIdentifier.screenRecording.rawValue,
                S0CategoryIdentifier.rest.rawValue
            ]
        )
        XCTAssertEqual(
            cards.map { $0.category },
            [.bigVideo, .screenRecording, .rest]
        )
        XCTAssertEqual(cards.map { $0.byteCount }, [300, 120, 580])

        // 占比手算：300／1000、120／1000、580／1000。夹具取值避开 .5 边界——
    ...
    func testIC162A_RestCardOmittedWhenZero() {
        ...
        XCTAssertFalse(cards.contains { $0.category == .rest })
    }
    ...
    func testIC162A_DefaultAndResolvedOpenID() {
        ...
        // 一张可点的都没有：不展开（IC-166：原「不可点的其余照片卡」改为两条尚未开始识别的类别）。
    ...
    /// IC-166：`fixtureCategories` 三条之后加一条「其余照片」行，`Σ c.bytes = LIB = 1000`
    /// （580 = 1000 − 300 − 0 − 120；占比同样不落在 .5 边界上）。
    private static let fixtureCategoriesWithRest = fixtureCategories + [
        S0CategorySnapshot(
            id: .rest,
            candidateCount: 9,
            candidateByteCount: 580,
            recognition: .settled
        )
    ]
